package org.jgroups.protocols.raft.example.blocks;
...
import org.jgroups.protocols.raft.internal.RaftHandle;
...
public class CounterService implements StateMachine, RAFT.RoleChange {
    ...
    public Counter getOrCreateCounter(String name, long initial_value) throws Exception {
        ...
        if (existing_value != null) counters.put(name, (Long) existing_value);
            ...
            if (retval instanceof Long) counters.put(name, (Long) retval);
        }
    ...
    public String printCounters() {
        return counters.entrySet().stream().collect(StringBuilder::new, (sb, entry) -> sb.append(entry.getKey()).append(" = ").append(entry.getValue())
                .append("\n"), (l, r) -> {
        }).toString();
    }
    ...
    public void set(String name, long newValue) throws Exception {
        invoke(Command.set, name, true, newValue);
    }
    ...
    public void dumpLog() {
        raft.logEntries((entry, index) -> {
            ...
            if (entry.internal()) {
                try {
                    InternalCommand cmd = (InternalCommand) Util.streamableFromByteBuffer(InternalCommand.class, entry.command(), entry.offset(), entry
                            .length());
    ...
    protected Object invoke(Command command, String name, boolean ignoreReturnValue, long... values) throws Exception {
